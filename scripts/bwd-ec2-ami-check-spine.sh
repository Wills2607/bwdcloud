...
SGID=$(aws ec2 describe-security-groups --filter "Name=description, Values=default VPC security group" --query "SecurityGroups[0].GroupId"  --output text)
#aws ec2 authorize-security-group-ingress --group-id $SGID --protocol tcp --port 22 --cidr 0.0.0.0/0

INSTANCEID=$(aws ec2 run-instances --image-id $AMIID --key-name BWD-EC2-KEY --instance-type t2.small --security-group-ids $SGID --subnet-id $SUBNETID --query "Instances[0].InstanceId" --output text)
